...
public class CFG {
	...
	public CFG(String cfgString) {
		...
		for (int i = 1; i <= splits.length; i++) {
			...
			if (!nodeSplits[0].equals("")) {
				node.setKind(Integer.parseInt(nodeSplits[0]));
			}
			allNodes.add(node);
		}
		for (int i = 1; i <= splits.length; i++) {
			String nodeStr = splits[(i - 1)];
			String[] nodeSplits = nodeStr.split(",", -1);
			CFGNode currentNode = (CFGNode) allNodes.get(i - 1);
				...
				for (int j = 6; j < nodeSplits.length; j++) {
					if (!nodeSplits[j].equals("")) {
						...
						if (!IDstr.equals("")) {
							...
							CFGNode successorNode = (CFGNode) allNodes.get(successorID - 1);
							currentNode.successors.add(successorNode);
							// currentNode.predecessors.add(currentNode); // This line has changed
		...
		entry = ((CFGNode) allNodes.get(0));
		exit = ((CFGNode) allNodes.get(allNodes.size() - 1));
	}
